...
export default class RequestHandler {
  ...
  static listenToDoc(col, doc) {
    return this.fs.collection(col).doc(doc);
  }
  ...
  static findDocuments(col, queryKey, query, queryValue, limit) {
    return this.fs
      .collection(col)
      .where(queryKey, query, queryValue)
      .limit(limit)
      .get();
  }
  ...
  static deleteKey(col, doc, keyToDel) {
    var tempObj = {};
    tempObj[`${keyToDel}`] = firebase.firestore.FieldValue.delete();
